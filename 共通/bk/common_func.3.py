...
def common_exe_sqlplus(connection_target,sql_file_path):

    output_file="sql_file_path.1.log"
    # 接続情報の取得
    ...
    command = f"start cmd /K sqlplus {connection_info['user']}/{connection_info['password']}@{connection_info['service_name']} @{sql_file_path} "

    try:
        # コマンドプロンプトを起動し、sqlplusを実行
        #subprocess.run(command, shell=True)
        # コマンドプロンプトを起動し、sqlplusを実行、出力をファイルにリダイレクト
        with open(output_file, "w") as output:
            subprocess.run(command, shell=True, stdout=output, stderr=output)
