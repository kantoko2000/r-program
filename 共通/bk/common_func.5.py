...
def common_kosin_sql_exe(connection_target,sql_file_path,input_content):
    ...
    while True:
        ...
        #SQLファイルから、SQLを読み込む
        sql = input("実行したいSQL文を入力してください (終了するには'EXIT'と入力): ")

        # 'EXIT'と入力されたらループを終了
        if sql.upper() == 'EXIT' or sql.upper() == 'exit' or sql.upper() == 'quit' or sql.upper() == 'QUIT':
            print("終了します...")
            break

        try:
            # 複数のSQL文を一度に実行する場合、execute()でまとめて実行
            sql_commands = sql.split(';')  # セミコロンで区切られたSQL文を分割
            for command in sql_commands:
                command = command.strip()  # 前後の空白を取り除く
                if command:  # 空のコマンドを避ける
                    cursor.execute(command)
                    
                    # SELECT文の場合、結果を取得
                    if command.lower().startswith("select"):
                        rows = cursor.fetchall()  # 結果を全て取得
                        if rows:
                            for row in rows:
                                print(row)  # 結果を行ごとに表示
                        else:
                        ...
                        print(f"{command} が正常に実行されました。")

            # 変更がある場合、ユーザーにコミットをするか聞く
            commit_choice = input("この変更をコミットしますか？ (y/n): ")
